...
mod test {
    ...
    async fn test_logging() -> Result<()> {
        ...
        let dropped_spans =
            recorder.get(&"layer.drop.span".into_counter()).unwrap_or(0);
        assert!(dropped_spans > 0, "No spans were dropped");
        let spans = recorder.get(&"layer.span".into_counter()).unwrap_or(0);
        assert!(spans > 0, "No spans were recorded");

        let dropped_events = recorder
            .get(&"layer.drop.event".into_counter())
            .unwrap_or(0);
        assert!(dropped_events > 0, "No events were dropped");
        let events = recorder.get(&"layer.event".into_counter()).unwrap_or(0);
