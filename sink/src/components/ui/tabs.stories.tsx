...
export const ShouldChangeTabs: Story = {
  ...
  play: async ({ canvas, step }) => {
    ...
    for (let i = 0; i < tabs.length; i++) {
      const name = tabs[i].textContent ?? ''

      await step(`click the '${name}' tab`, async () => {
        await userEvent.click(tabs[i])
        await waitFor(() =>
          expect(tabs[i]).toHaveAttribute('aria-selected', 'true'),
        )
        await expect(await canvas.queryByRole('tabpanel', { name })).toBeVisible()
      })
